...
#define DEVICE_NAME                         "24Patch3"                            /**< Name of device. Will be included in the advertising data. */
...
#define MIN_HEART_RATE                      140//140                                     /**< Minimum heart rate as returned by the simulated measurement function. */
#define MAX_HEART_RATE                      300//300                                     /**< Maximum heart rate as returned by the simulated measurement function. */
...
static const nrf_drv_twi_t m_twi = NRF_DRV_TWI_INSTANCE(TWI_INSTANCE_ID); /* TWI instance. 180821*/
...
static uint16_t updateFakePPG(void)
{
	static uint8_t fakePPG_count = 0;
	if(fakePPG_count > 6)
	{
		fakePPG_count = 0;
	}
	fakePPG_count++;
	return dataPPG_test[fakePPG_count-1];
}
...
static void battery_level_update(void)
{
    ...
    //battery_level = (uint8_t)sensorsim_measure(&m_battery_sim_state, &m_battery_sim_cfg);
		//battery_level = (uint8_t)updateFakePPG();
		//battery_level = (uint8_t)IR_LED;
	if(validHeartRate == 1)
	{
		if(heartRate < 120 && heartRate > 80)
		{
			battery_level = heartRate;
		}
...
static void heart_rate_meas_timeout_handler(void * p_context)
{
	...
	//heart_rate = beatAvg;
	//heart_rate = updateFakePPG();
	heart_rate = IR_LED;
	//heart_rate = IR_AC_Signal_Current;
	
	//heart_rate = 0xAABBCCDD;
	//NRF_LOG_INFO("updateFakePPG : %d", updateFakePPG());
	//if(dataPPG_count > 6)
	//{
	//	dataPPG_count = 0;
	//}
	//heart_rate = dataPPG_test[dataPPG_count];
	//dataPPG_count++;
	//NRF_LOG_INFO("heart_rate_meas_timeout_handler[heart_rate]: %d ", heart_rate);
	//NRF_LOG_INFO("heart_rate_meas_timeout_handler[IR_LED]: %d ", IR_LED);
	//NRF_LOG_INFO("heart_rate_meas_timeout_handler[IR_LED16]: %d ", (uint16_t)IR_LED);
...
static void sensor_contact_detected_timeout_handler(void * p_context)
{
	...
	err_code = spo2_send(&m_hrs, spo2_);
    if ((err_code != NRF_SUCCESS) &&
        (err_code != NRF_ERROR_INVALID_STATE) &&
        (err_code != NRF_ERROR_RESOURCES) &&
        (err_code != NRF_ERROR_BUSY) &&
...
static void application_timers_start(void)
{
    ...
    APP_ERROR_CHECK(err_code);
	NRF_LOG_INFO("++++++++++++++++++application_timers_start++m_battery_timer_id+++++++++");				
    err_code = app_timer_start(m_heart_rate_timer_id, HEART_RATE_MEAS_INTERVAL, NULL);
    APP_ERROR_CHECK(err_code);
	NRF_LOG_INFO("++++++++++++++++++application_timers_start++m_heart_rate_timer_id+++++++++");				
    err_code = app_timer_start(m_rr_interval_timer_id, RR_INTERVAL_INTERVAL, NULL);
    APP_ERROR_CHECK(err_code);
	NRF_LOG_INFO("++++++++++++++++++application_timers_start++m_rr_interval_timer_id+++++++++");				
    err_code = app_timer_start(m_sensor_contact_timer_id, SENSOR_CONTACT_DETECTED_INTERVAL, NULL);
    APP_ERROR_CHECK(err_code);
	NRF_LOG_INFO("++++++++++++++++++application_timers_start++m_sensor_contact_timer_id+++++++++");					
}
...
{
	//NRF_LOG_INFO("in app_timer_cnt_get() : %d", app_timer_cnt_get());
...
static void max30102_init(void)
{
	...
	writeRegister(REG_FIFO_CONFIG, 0x0f);//sample avg = 4, fifo rollover=false, fifo almost full = 17 [0x08 0000 1111] 
	...
	writeRegister(REG_LED1_PA, 0x24);//about 11mA : Choose value for ~ 7mA for LED1 [0x0C 0010 0100]
	writeRegister(REG_LED2_PA, 0x24);//about 11mA : Choose value for ~ 7mA for LED2 [0x0D 0010 0100]
...
static void calHR(void)
{
	//NRF_LOG_INFO("checkForBeat(IR_LED) : %d", checkForBeat(IR_LED));
	
	//read heartrate
	if(checkForBeat(IR_LED) == true)
	{
...
static void calSPO2(void)
{
	...
	NRF_LOG_INFO("===============================HR: %d", heartRate);
	NRF_LOG_INFO("===============================beatAvg: %d", beatAvg);
}
...
void gpio_init()
{
   ...
   there are no external pullup resistors on the nRF52 development board. */
	...
	drv_gpio_inpin_cfg(PPG_INT, in_cfg, DRV_GPIO_NO_PARAM_PTR);
}
...
int main(void)
{
	...
	//gpio init
	//gpio_init();
	//NRF_LOG_INFO("ini PPG_INT : %d", drv_gpio_inpin_get(PPG_INT, &PPG_INT_value));

	//twi init
	twi_init();//nrf_drv_twi_init(), nrf_drv_twi_enable()
	max30102_init(); //init max30102 resistor
	//NRF_LOG_INFO("MAX INI PPG_INT : %d", drv_gpio_inpin_get(PPG_INT, &PPG_INT_value));
